  ...
  <div class="projects-container">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">Projects Management</h2>
        <router-link to="/projects/create" class="btn btn-success">Add Project</router-link>
      </div>
      ...
      <template v-else>
        <table v-if="projects.length" class="table">
          ...
          <tbody>
            <tr v-for="project in projects" :key="project.id">
              ...
              <td class="actions">
                <router-link :to="`/projects/${project.id}/edit`" class="btn btn-primary btn-sm">Edit</router-link>
        ...
        <div v-else class="no-data">
          No projects found. <router-link to="/projects/create">Create one</router-link>
        </div>
...
const loadProjects = async () => {
  ...
  error.value = ''
  try {
    const response = await projectsAPI.listProjects(currentPage.value, pageSize)
...
const toggleProject = async (project) => {
  try {
    await projectsAPI.enableDisableProject(project.id, !project.status)
    await loadProjects()
  } catch (err) {
