<template>
  <div class="statistics-page">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">{{ $t('dashboard.clientStatus') }}</h1>
        <p class="page-subtitle">{{ $t('dashboard.totalClients') }}</p>
      </div>
      <button 
        @click="refreshData" 
        class="refresh-btn"
    ...
    <div v-else class="statistics-content">
      ...
      <div class="metrics-group">
        <div class="metric-card highlight">
          <span class="metric-label">{{ $t('dashboard.totalClients') }}</span>
          <span class="metric-value">{{ clientStats.total }}</span>
        </div>
        <div class="metric-card">
          <span class="metric-label">{{ $t('dashboard.active') }}</span>
          <span class="metric-value success">{{ clientStats.active }}</span>
        </div>
        <div class="metric-card">
          <span class="metric-label">{{ $t('dashboard.inactive') }}</span>
          <span class="metric-value danger">{{ clientStats.inactive }}</span>
        </div>
      </div>

      <!-- Chart Section -->
      <div class="chart-section">
        <h2 class="section-title">{{ $t('dashboard.clientStatus') }}</h2>
        <DoughnutChart
          :data="clientStatusChart"
          :title="$t('dashboard.clientStatus')"
          :loading="loading"
        />
      </div>
    </div>
